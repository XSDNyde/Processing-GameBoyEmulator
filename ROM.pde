...
class ROM_DMGBootstrapMemory implements BusSubscriber
{
  
  /* Bootstrap links:
   * Binary: https://gbdev.gg8.se/files/roms/bootroms/dmg_boot.bin
   * Disassembly: https://codeberg.org/ISSOtm/gb-bootroms/src/branch/master/src/dmg.asm
   * Disassembly: https://www.neviksti.com/DMG/DMG_ROM.asm
   */
  ...
  {
    assert address >= 0 && address < data.length : "Address " + address + " is outside the BOOTSTRAP Memory!";
    if( DEBUG_OUTPUT ) println( "[DEBUG] ROM_DMGBootstrapMemory READ " + HEX( data[ address ] ) + " FROM " + HEX2( address ) );
  ...
  {
    assert false : "Writing to BOOTSTRAP ROM is technically impossible!";
    
    assert address >= 0 && address < data.length : "Address " + address + " is outside the BOOTSTRAP Memory!";
    if( DEBUG_OUTPUT ) println( "[DEBUG] ROM_DMGBootstrapMemory WRITE " + HEX( value ) + " TO " + HEX2( address ) );
